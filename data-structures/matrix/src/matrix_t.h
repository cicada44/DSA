...
public:
    explicit matrix(size_t rows = 0, size_t cols = 0,
        ...
        : m_Rows(rows),
          m_Cols(cols),
          m_Capacity(rows * 2 * cols),
          m_Allocator(allocator) {
        if (rows == 0 || cols == 0)
            throw std::runtime_error(
                "Matrix can't contain zero rows or columns");
    ...
    matrix(const std::initializer_list<std::initializer_list<T>>& il)
        ...
        for (const auto& il_i : il) {
            if (il_i.size() != stable_cols)
                throw std::runtime_error(
                    "Matrix must contain same number of elements in each "
                    "column");
    ...
    // C++23
    // inline T operator[](size_t i, size_t j) noexcept { return
    // m_Data[getIdx(i, j)]; }

    inline const T& at(size_t i, size_t j) const {
        if (i >= m_Rows || j >= m_Cols)
            throw std::out_of_range("Indexes is out of range");
    ...
    inline void resize_row(size_t count, const T& defaultValue = T{}) {
        ...
        if (count > m_Rows) {
            if (count * m_Cols > m_Capacity)
                reserve(m_Capacity + count * m_Cols);
            for (size_t i = m_Rows; i != count; ++i)
                for (size_t j = 0; j != m_Cols; ++j)
                    m_Data[getIdx(i, j)] = defaultValue;
        }
        m_Rows = count;
    }
    ...
    inline void resize_col(size_t count, const T& defaultValue = T{}) {
        ...
        for (size_t i = 0; i < m_Rows; ++i) {
            for (size_t j = 0; j < std::min(m_Cols, count); ++j)
                newStorage[getIdx(i, j, count)] = m_Data[getIdx(i, j)];
            for (size_t j = m_Cols; j < count; ++j)
                newStorage[getIdx(i, j, count)] = defaultValue;
        }
    ...
    inline void insert_row(size_t pos, const std::initializer_list<T>& il) {
        if (il.size() != m_Cols)
            throw std::runtime_error(
                "The size of inserted row don't fit matrix");
        if (pos > m_Rows)
            throw std::out_of_range("Can't insert row after matrix end");
        if (m_Capacity < (m_Rows + 1) * m_Cols)
            reserve((m_Capacity + m_Cols) * 2);
    ...
    inline void insert_col(size_t pos, const std::initializer_list<T>& il) {
        if (il.size() != m_Rows)
            throw std::runtime_error(
                "The size of inserted column don't fit matrix");
        if (pos > m_Cols)
            throw std::out_of_range("Can't insert column after matrix end");
        if (m_Capacity < m_Rows * (m_Cols + 1))
            reserve((m_Capacity + m_Rows) * 2);
        matrix<T> temp(
            m_Rows,
            m_Cols + 1);  // Evgeny Sergeevich, I HATE THIS, but i was forsed!!!
        for (size_t i = 0; i < m_Rows; ++i)
            for (size_t j = 0; j < m_Cols; ++j)
                temp.at(i, j + ((j >= pos) ? 1 : 0)) = m_Data[getIdx(i, j)];
        for (size_t i = 0; i < m_Rows; ++i) temp.at(i, pos) = *(il.begin() + i);
        for (size_t i = 0; i < m_Rows; ++i)
            for (size_t j = 0; j < m_Cols + 1; ++j)
                m_Data[i * (m_Cols + 1) + j] = temp.at(i, j);
        ++m_Cols;
    }

    inline void push_back_row(const std::initializer_list<T>& il) {
        insert_row(m_Rows, il);
    }

    inline void push_back_col(const std::initializer_list<T>& il) {
        insert_col(m_Cols, il);
    }
    ...
    void erase_row(size_t idx) {
        ...
        for (size_t i = idx; i < m_Rows - 1; ++i)
            for (size_t j = 0; j < m_Cols; ++j)
                m_Data[getIdx(i, j)] = m_Data[(i + 1) * m_Cols + j];
        m_Rows--;
    }

    void erase_col(size_t idx) {
        if (idx > m_Cols)
            throw std::out_of_range("Can't delete column after matrix end");
        for (size_t i = 0; i != m_Rows * m_Cols; ++i)
            if ((idx == m_Cols - 1 && i % idx == 0 && i != 0) ||
                (idx != m_Cols - 1 && i % (m_Cols - 1) == idx))
                for (size_t j = i; j != m_Rows * m_Cols; ++j)
                    m_Data[j] = m_Data[j + 1];
        --m_Cols;
    }
    ...
    public:
        ...
        bool operator==(matrix_iter iter) noexcept {
            return iter.current_ == current_;
        }
...
protected:
    inline size_t getIdx(size_t i, size_t j) const noexcept {
        return (i * m_Cols + j);
    }

    inline size_t getIdx(size_t i, size_t j, size_t cols) const noexcept {
        return (i * cols + j);
    }
...
bool operator==(const matrix<T>& m1, const matrix<T>& m2) {
    if (m1.size() != m2.size() || m1.rows() != m2.rows() ||
        m1.cols() != m2.cols())
        return false;
